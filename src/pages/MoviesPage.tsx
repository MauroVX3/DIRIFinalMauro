  ...
  return (
    <section className="page-section container">
      <div className="page-heading">
        <h1>{intl.formatMessage({ id: 'movies.title' })}</h1>
        <p>{intl.formatMessage({ id: 'movies.subtitle' })}</p>
      </div>
      ...
      {hasMore &&
        <div className="load-sentinel">
          <button className="button-secondary" type="button" disabled={loading} onClick={() => dispatch(fetchMovies({ search, page: page + 1 }))}>{intl.formatMessage({ id: loading ? 'movies.loadingMore' : 'common.loadMore' })}</button>
        </div>}
    </section>
  )
}
