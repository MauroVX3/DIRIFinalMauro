  ...
  return (
    <section className="auth-page">
      <form className="form-card" onSubmit={submit}>
        ...
        <label>{intl.formatMessage({ id: 'auth.email' })}<input required type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label>
        <label>{intl.formatMessage({ id: 'auth.password' })}<input required minLength={6} type="password" autoComplete={mode === 'login' ? 'current-password' : 'new-password'} value={password} onChange={(event) => setPassword(event.target.value)} /><small>{intl.formatMessage({ id: 'auth.passwordHelp' })}</small></label>
